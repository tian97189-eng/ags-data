...
vi.mock('../../lib/sync', () => ({
  ...
  getSavedServerURL: mocks.getSavedServerURL,
  saveServerURL: mocks.saveServerURL,
...
beforeEach(async () => {
  ...
  mocks.getSavedServerURL.mockReset();
  mocks.saveServerURL.mockReset();
  ...
  mocks.getSavedServerURL.mockResolvedValue('');
  mocks.saveServerURL.mockResolvedValue(undefined);
...
describe('CloudSyncSettings 云同步面板', () => {
  ...
  it('填入 AppID 与 AppKey 点启用 → 保存并连接', async () => {
    render(<CloudSyncSettings />);
    const idInput = await screen.findByPlaceholderText(/应用凭证/);
    await userEvent.type(idInput, 'my-app-id-123');
    const keyInput = await screen.findByPlaceholderText(/Master Key/);
    await userEvent.type(keyInput, 'my-app-key');
    await userEvent.click(screen.getByText('启用云同步'));
    await waitFor(() => {
      expect(mocks.saveEnvId).toHaveBeenCalledWith('my-app-id-123');
      expect(mocks.saveAccessKey).toHaveBeenCalledWith('my-app-key');
      expect(mocks.initSync).toHaveBeenCalledWith('my-app-id-123', 'my-app-key', '');
    });
  });

  it('未填 AppKey 点启用会提示且不连接', async () => {
    render(<CloudSyncSettings />);
    const idInput = await screen.findByPlaceholderText(/应用凭证/);
    await userEvent.type(idInput, 'my-app-id-123');
    await userEvent.click(screen.getByText('启用云同步'));
