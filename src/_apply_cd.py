import re
p = 'src/pages/Overview/index.tsx'
with open(p, 'r', encoding='utf-8') as f:
    t = f.read()

# 1. import 加 useEffect
...
assert old_state in t, 'old_state not found'
...
assert old_ui in t, 'old_ui not found'
t = t.replace(old_ui, new_ui, 1)

# 4. 删 cdDays
t = t.replace("  const cdDays = countdown ? daysUntil(countdown.date) : null;\n", "", 1)

# 5. 加 CountdownItem / CountdownAddForm 组件
components = '''/** 倒计时单项：默认显示剩余天数，点编辑切换到行内表单；保存回写或删除本项 */
...
function CountdownAddForm({ onAdd }: { onAdd: (label: string, date: string) => void }) {
  ...
  if (!open) {
    return (
      <button type="button" onClick={() => setOpen(true)} className="mt-3 text-[12px] text-brand-700 hover:underline">
        + 添加倒计时事件
      </button>
    );
  }
  return (
    ...
    >
      <input
        ...
        placeholder="事件名，比如：答辩"
        className="w-full text-[13px] border border-slate-200 dark:border-slate-700 dark:bg-slate-900 rounded px-2 py-1"
      />
      <input
        type="date"
        aria-label="新事件日期"
      ...
      <div className="flex gap-1.5">
        ...
        <button
          ...
          className="px-2 py-1 border border-slate-200 dark:border-slate-700 rounded text-[12px]"
        >
          取消
        </button>
      </div>
...
with open(p, 'w', encoding='utf-8') as f:
    f.write(t)
print('done. CountdownItem:', t.count('function CountdownItem'), 'CountdownAddForm:', t.count('function CountdownAddForm'))
